      ...
      >
        <div class="item_card" @click="$router.push({name: 'personDetail', params: item, query: {type: 'invite'}})" v-for="item in listState.list" :key="item.appid">
          <div class="__card_item">
            <span class="__flex_1 __header">{{item.visitorName}} {{item.visitorTel}} <van-icon style="margin: 4px 0 0 6px;" size="16px" name="phone-circle-o" /></span>
            <span class="__align_r"><van-tag round type="primary" :color="statusColorMap[item.visitStatus] || '#07c160'"> {{statusMap[item.visitStatus] || '--'}} </van-tag></span>
          </div>
          <div class="__card_item">
            <span class="__flex_1">{{item.visitorCompanyName || '--'}}</span>
            <span class="__align_r" style="width: 150px">{{item.visitTime}}</span>
          </div>
          <!-- <div class="__card_item">
            {{item.employeeName}}
          </div> -->
